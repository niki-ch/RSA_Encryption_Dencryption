// RSA_Encryption_Dencryption
// Encryption and Dencryption of a user's message 
...
void generate_primeNum(mpz_t p, mpz_t q){
    ...
    //if p=0 or q=0, then executes again the above algorithm
...
void check_primality(mpz_t p){
    ...
    mpz_init(m); //initialize variable m to zero
...
int main(){
    ...
    fgets(message, sizeof(message), stdin); //function that read the user's message, store it to the array message[] and print it
    printf("\n");
    ...
    //print user's message, every character of message is converted to integer according to ASCII array
    ...
    //print user's message to the original form
    for(i=0; i<(length-1); i++){
        printf("%c", message[i]);
    }
    printf("\n\n");
